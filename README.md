...
- **AM调制**: 标准AM信号 s(t) = [1 + μ·m(t)]·cos(2πfc·t)
- **三种解调算法**:
  - 包络检波法（非相干解调）
  - Hilbert变换包络检波
  - 相干解调法（同步检波）
- **性能分析**: SNR对比评估
- **调制指数控制**: 支持0-1.5范围（含过调制测试）
...
- `am_signal.txt/csv` - AM调制信号及载波
- `am_demodulated.csv` - 三种解调方法对比
- `am_*.png` - 可视化图表（调制过程、解调对比、频谱）
...
📖 详细文档: 
- [README-AM.md](README-AM.md) - 完整技术文档
- [QUICKSTART-AM.md](QUICKSTART-AM.md) - 快速入门
- [AM-SUMMARY.md](AM-SUMMARY.md) - 项目总结
...
make fft2d             # 编译2D FFT程序
make kspace_to_image   # 编译K空间重建程序
make fm_signal         # 编译FM调频程序
make am_signal         # 编译AM调幅程序
